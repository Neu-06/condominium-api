...
class LoginJWTView(APIView):
    ...
    def post(self, request):
        ...
        if not user:
            ...
            registrar_bitacora(usuario, 'LOGIN_FALLIDO', f'Intento de login fallido desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
        ...
        registrar_bitacora(user, 'LOGIN', f'Login exitoso desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
...
class PerfilView(APIView):
    ...
    def get(self, request):
        ...
        user = request.user
        registrar_bitacora(user, 'PERFIL', f'Acceso a perfil desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
...
class UsuarioViewSet(viewsets.ModelViewSet):
    queryset = Usuario.objects.all().select_related('rol', 'residente', 'personal').order_by('id')
...
class RegistroView(APIView):
    ...
    def post(self, request):
        ...
        if ser.is_valid():
            user = ser.save()
            registrar_bitacora(user, 'REGISTRO', f'Registro exitoso desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                usuario.save()
                registrar_bitacora(usuario, 'RECUPERAR_PASSWORD', f'Contraseña recuperada desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
                ...
                return Response({'detail': 'Si el correo existe, se enviará la nueva contraseña'})
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                usuario.token_expira = timezone.now() + timezone.timedelta(hours=1)  # Expira en 1 hora
                usuario.save()
...
Has solicitado recuperar tu contraseña. Usa el siguiente token para crear una nueva contraseña:
...
Equipo Smart Condominium
                    ...
                    registrar_bitacora(usuario, 'SOLICITAR_RECUPERACION', f'Token de recuperación enviado desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
                    ...
                    # Si falla el email, devolver el token (solo para desarrollo)
                    return Response({
                        'detail': 'Error al enviar email. Token temporal:',
        ...
        # Siempre devolver respuesta exitosa para no revelar usuarios existentes
        return Response({
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                registrar_bitacora(usuario, 'RECUPERACION_EXITOSA', f'Contraseña recuperada exitosamente desde {request.META.get("REMOTE_ADDR", "IP desconocida")}', request)
